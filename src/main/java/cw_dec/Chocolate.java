...
dimension n x m into small squares. Each square is
of size 1x1 and unbreakable. Implement a function
...
just one break, but for size 3 x 1 you must do two
breaks.

If input data is invalid you should return 0 (as in
no breaks are needed if we do not have any chocolate
to split). Input will always be a non-negative integer.
 */
public class Chocolate {
  public static long breakChocolate(long n, long m) {
    return (n > 0 && m > 0) ? (n * m - 1L) : 0;
  }
}
